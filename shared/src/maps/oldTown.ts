...
/**
 * MAP 1 — Kadalimukku Old Town. A compact traditional junction town in the monsoon.
 *
...
// ── Interiors ──────────────────────────────────────────────────────────────
for (const dx of [160, 300, 440]) for (const dy of [180, 250, 320]) b.deco('desk', dx, dy, 70, 24, true);
b.deco('desk', 170, 104, 100, 26, true); // teacher's table
b.deco('counter', 890, 190, 150, 26, true);
b.deco('bench', 900, 300, 90, 20, true);
b.deco('bench', 1010, 330, 90, 20, true);
b.deco('panel', 2010, 250, 70, 60, true); // transformer
b.deco('panel', 1886, 120, 30, 34, false); // main power panel
b.deco('panel', 2106, 120, 30, 34, false); // fuse board
b.deco('bench', 160, 780, 120, 24, true); // thinnai
b.deco('shelf', 420, 620, 90, 24, true);
b.deco('workbench', 1860, 620, 160, 30, true);
b.deco('scooter', 2150, 760, 70, 34, true);
b.deco('generator', 2200, 620, 70, 60, true);
b.deco('pump', 1000, 1240, 80, 60, true);
b.deco('panel', 1166, 1226, 30, 30, false); // valve A
b.deco('stage', 1960, 1440, 300, 50, true);
b.deco('radio', 2236, 1240, 32, 24, false);

// ── Bus stop & junction ────────────────────────────────────────────────────
b.deco('shelter', 1300, 370, 260, 14, true);
b.deco('bench', 1330, 395, 200, 16, true);
b.deco('auto', 1590, 395, 70, 44, true);
b.deco('bell', 1190, 630, 20, 20, true);
b.deco('cctv', 1320, 575, 14, 14, true);
b.deco('bicycle', 760, 560, 44, 16, true);
b.deco('well', 880, 700, 50, 50, true);

// ── Residential lane ───────────────────────────────────────────────────────
b.deco('house_block', 80, 1220, 220, 160, true);
b.deco('house_block', 360, 1220, 200, 160, true);
b.deco('house_block', 80, 1440, 200, 120, true);
b.deco('house_block', 600, 1420, 220, 140, true);
b.deco('banana', 380, 1450, 40, 40, false);

// ── Market ────────────────────────────────────────────────────────────────
b.deco('stall', 1320, 1230, 120, 50, true);
b.deco('stall', 1500, 1230, 120, 50, true);
b.deco('stall', 1320, 1370, 120, 50, true);
b.deco('stall', 1500, 1370, 120, 50, true);
b.deco('stall', 1680, 1250, 90, 140, true);
b.deco('drum', 1760, 1470, 36, 36, true);
...
const canal = { x: 0, y: 1000, w: W, h: 90 };
b.water(canal, bridges);

export const KADALIMUKKU_OLD_TOWN: GameMapDef = b.finish({
  id: 'kadalimukku_old_town',
  nameKey: 'map.kadalimukku_old_town',
  descriptionKey: 'map.kadalimukku_old_town.desc',
  theme: { id: 'monsoon_town', palette: {}, background: '#0e1512', rain: true, lightning: true },
  width: W,
  height: H,
  spawn: { x: 1200, y: 700 },
  spawnRadius: 95,
  outdoorZoneId: 'streets',
  roads: [
    { x: 0, y: 460, w: W, h: 80 },
  ...
  meetingLocations: [{ id: 'alarm_bell', x: 1200, y: 640, zoneId: 'junction' }],
  objectives: [
    { id: 'antidote_school', kind: 'antidote_part', x: 560, y: 380, zoneId: 'school' },
    { id: 'antidote_house', kind: 'antidote_part', x: 300, y: 700, zoneId: 'house' },
    { id: 'antidote_workshop', kind: 'antidote_part', x: 2060, y: 830, zoneId: 'workshop' },
    { id: 'antidote_market', kind: 'antidote_part', x: 1470, y: 1325, zoneId: 'market' },
  ],
  cameras: [],
  securityConsoles: [],
  specialRules: { visionMultiplier: 1, ruleKeys: ['maprule.monsoon'] },
  supportedModes: ['classic', 'hunt', 'infection'],
});
